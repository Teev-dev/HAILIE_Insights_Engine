...
class GetProviderDatasetTypeTests(unittest.TestCase):
    ...
    def test_get_lcra_dataset_type(self):
        ...
        self.assertEqual(self.data_processor.get_provider_dataset_type(lcra_example_name), "LCRA", "Failed to fetch provider type for LCRA provider")

    def test_get_lcro_data_type(self):
        lcho_example_name = "Cross Keys Homes Limited - LCHO"
        self.assertEqual(self.data_processor.get_provider_dataset_type(lcho_example_name), "LCHO", "Failed to fetch provider type for LCHO provider")

    def test_get_combined_dataset_type(self):
        combined_example_name = "North London Muslim Housing Association Limited - COMBINED"
        self.assertEqual(self.data_processor.get_provider_dataset_type(combined_example_name), "COMBINED", "Failed to fetch provider type for Combined provider")
