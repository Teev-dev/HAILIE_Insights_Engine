...
Spot-checks provider scores in DuckDB against expected invariants.

Run manually: `python3 validate_etl.py`
Intentionally excluded from the Docker image (see .dockerignore) — this is
a dev/CI tool, not a runtime module.
"""
...
def validate():
    ...
    # 6. Dataset mapping integrity: COMBINED and LCRA/LCHO should not overlap,
    # but a provider CAN legitimately appear in both LCRA and LCHO.
